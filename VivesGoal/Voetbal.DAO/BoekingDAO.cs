...
namespace Voetbal.DAO
{
    public class BoekingDAO
    {
        ...
        public IEnumerable<Boeking> BoekingenKlant(string id)
        {
            using (var db = new VoetbalClubEntities())
            {
                return db.Boeking.Where(b => b.klant == id).Include( b =>b.Wedstrijd1.Club).Include(b=> b.Wedstrijd1.Club1).Include(b=> b.Wedstrijd1).Include( b=> b.ZitPlaats1.Vak).ToList();
            }
        }
